...
## 四點五、「中間層」跟「抽象層」不是同一件事

這兩個詞常被混用，但它們是**不同維度**的分類，可以同時成立、也可以互相無關。

| | 中間層 | 抽象層 |
|---|---|---|
| 定義依據 | **位置**——在呼叫鏈裡夾在 A 跟 C 中間 | **功能**——介面藏住的實作複雜度，有沒有比介面本身複雜 |
| 判斷方式 | 看架構圖，純結構性 | 看本文的深模組／淺模組判準：介面窄、功能深＝好的抽象 |
| 好壞 | 中性，本身不代表好或壞 | 有好壞之分——淺模組就是本文在講的抽象化陷阱 |

關鍵是這兩者互相獨立：**一個中間層可以是好的抽象層，也可以只是個空殼。**

- 一個典型的 `service` 層（route → service → db client）：它是中間層，同時也是好的抽象層——窄介面（例如 `login(data)`）背後藏了查詢、雜湊比對、驗證、錯誤格式化，呼叫者完全不用知道這些細節。
- 本文開頭的 `EmailValidationStrategy`：它也是中間層（夾在 `checkEmail` 跟正規表達式中間），但不是好的抽象層——介面（`.validate()`）跟它藏的東西（一行正規表達式）一樣淺，只是把同一行邏輯搬到更難找的地方。

所以「這段程式碼 import 了層層堆疊的模組，是不是不好維護？」這個問題，真正該問的不是「這是不是中間層」（分層架構本來就會有中間層，這是中性的），而是**每一層是不是真的抽象層**——介面窄的背後，有沒有藏住讀者不需要知道的真實複雜度。如果一個「中間層」只是把呼叫原封不動轉手給下一層，沒有藏住任何東西，那它既不是有害的耦合，也不是有價值的抽象，只是純粹的間接層（indirection）——多一層要跳，但沒換到任何好處。
