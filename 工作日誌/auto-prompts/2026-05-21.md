...
我先用 schedule 技能來瞭解這個環境的排程代理（routines）系統，因為你的「auto-prompts」很可能是透過排程代理或 cron 設定的。
...
| `summarize_tool` | `name` 是工具名,依工具挑代表性參陣列摘要 |
...
- **獨立日曆**:建一個叫「Claude 工作日誌」的專屬 Google Calendar,不汙染你的主日曆。
...
現在補上 `requirements.txt`、更新 `.gitignore`(把秘密檔擋掉),並查 Abby-notes 的 Python 筆記資料夾。
...
程式碼那邊我已經全部寫好了:`claude_log.py`(共用解析+你的註解)、`sync-to-calendar.py`(同步主程式)、精簡後的 `show-my-prompts.py`、`config.example.json`、`requirements.txt`,`.gitignore` 也擋好秘密檔了。
...
**為什麼 IDE 看不到?** 因為我把 `service_account.json` 加進了 `.gitignore`(故意的 —— 這樣這把秘密金鑰永遠不會被 commit 上 GitHub)。VS Code 這類 IDE 預設會把 `.gitignore` 忽略的檔案**變灰或藏起來**,所以你才以為它不見了。它在硬碟上好好的。
...
1. **venv 是隔離的 Python 環境**,套件裝在裡面不會汙染系統全域 Python,彼此乾淨。
...
{% raw %}- `PowerShell`: docker ps --filter "publish=5433" --format "table {{.Names}}\t{{.Image}}\t{{.Sta...{% endraw %}
...
{% raw %}（如果 container 真正的名稱是 `abby-rag-postgres`，把 `pgvector-test` 換掉。先跑 `docker ps --format "{{.Names}}"` 看清楚）{% endraw %}
...
那個 `PS C:\...>` 就是「提示字元」，遊標在後面閃。**直接把指令貼進去按 Enter** 就會執行。
...
`\dt` 看錶清單（要有 `items_st`），`\dx` 看擴充清單（要有 `vector`）。
...
{% raw %}## 15:53 — docker ps --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}" 不是阿 你要幫我檢查我到底是用哪一個container阿{% endraw %}
...
**Tools:**
{% raw %}- `PowerShell`: docker ps --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}\t{{.Image}}"{% endraw %}
{% raw %}- `PowerShell`: docker ps -a --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}\t{{.Image}}"{% endraw %}
{% raw %}- `PowerShell`: docker inspect abby-rag-postgres --format '{{range .Config.Env}}{{println .}}{{e...{% endraw %}
...
{% raw %}- `PowerShell`: docker inspect abby-rag-postgres --format '{{index .Config.Labels "com.docker.co...{% endraw %}
...
這會直接列出你筆記的所有頂層分類（Python、Golang、RAG、backend...），一目瞭然你「整理過哪些主題」。
...
- **Model（embedding 模型)** → 跑在 **Python 行程裡**,直接用你 CPU/GPU 算。下載過就快取在本機(`~/.cache/huggingface`),跟 Docker 一點關係都沒有。
...
{% raw %}- `PowerShell`: Write-Output "===== 所有 container（含停止）====="; docker ps -a --format "table {{.Nam...{% endraw %}
...
{% raw %}- `Bash`: docker ps --format "table {{.Names}}\t{{.Image}}\t{{.Ports}}"{% endraw %}
{% raw %}- `Bash`: docker ps -a --format "table {{.Names}}\t{{.Status}}\t{{.Image}}"{% endraw %}
{% raw %}- `Bash`: docker images --format "table {{.Repository}}:{{.Tag}}\t{{.CreatedSince}}"{% endraw %}
...
`psycopg2.connect(...)` 回傳的是 **connection**（連線本身），但連線**不能直接執行 SQL**。要先開一個 **cursor**（遊標）才能跑：

```python
self.conn = psycopg2.connect(...)        # 拿到連線

with self.conn.cursor() as cur:          # 開遊標
    cur.execute("SELECT 1")              # 用遊標執行 SQL
    rows = cur.fetchall()                # 用遊標拿結果
```
...
**tuple 的中文是「元組」**（有時叫「陣列」）。它是一種**資料結構**，不是「傳入參數」的意思。
...
全部到齊。先公佈 RAG 找到的答案，再答其餘問題，最後補筆記。
...
| `backend/member_company.md` line 39 | 「模組級文檔字串（module-level docstring）是 Python 的標準寫法」 |
...
後 ingest.py   →  把 4172 個 chunk 寫進去（搬傢俱進房子）
```

**順序不能反**：表還不存在就 ingest，會噴 `relation "chunks" does not exist`（房子還沒蓋就要搬傢俱進去）。
...
3. python scripts/ingest.py    ← 寫入向量資料（搬傢俱）
```
...
from src.embedder import Embedder        # 自己寫（但裡麵包外部 model）
```
...
瞭解，全部都 commit，每個各一個。先用 quotepath off 取得正確的 UTF-8 檔名、並展開資料夾看實際檔案數，才能規劃。
