...
我們目前用的是**緊耦合**做法（業務程式碼裡直接呼叫 log service）。是否需要改為鬆耦合需要團隊另外評估，以下列出兩邊的考量供參考：

| 考量 | 緊耦合 | 鬆耦合 |
|---|---|---|
| 維護成本 | 低，不需要額外基礎設施 | 高，需要 message queue / CDC 等 |
| 效能影響 | log 寫入會佔用業務請求的時間 | 非同步不阻塞業務請求 |
| 失敗處理 | log 掛了可能影響業務操作 | log 掛了不影響業務，但可能靜默丟失 |
| 可靠性 | 寫入失敗立即發現 | 需要額外監控確保不漏 |
