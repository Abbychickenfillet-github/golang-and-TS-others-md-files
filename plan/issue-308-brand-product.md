...
> **✅ 決議：採用做法 1（一對多關聯 + 多態圖片表）。** 搭配[議題三](#議題三通用化-event_image-表--多態圖片表)的多態圖片表設計，用單一 `image` 表透過 `entity_type` + `entity_id` 管理所有實體的圖片。

### 做法 1：獨立的圖片表（一對多關聯 + 多態圖片表）⬅ 採用

```sql
-- 搭配議題三的多態圖片表，不用另建 product_image，直接用 image 表
INSERT INTO image (id, entity_type, entity_id, image_url, image_type, display_order)
VALUES (UUID(), 'product', '{product_id}', 'https://s3.../img1.jpg', 'main', 0);
```

如果不做多態圖片表，也可以獨立建 `product_image` 表：
...
- 我們自己的 `event_image` 表就是這個模式（`event_id` + `image_url`）→ 建議通用化為多態圖片表，見[議題三](#議題三通用化-event_image-表--多態圖片表)

### 做法 2：JSON 欄位（在 product 表加一個 JSON column）
...
| | 獨立圖片表（做法 1）⬅ 採用 | JSON 欄位（做法 2） | 多個固定欄位（做法 3） |
|---|---|---|---|
| 改動大小 | 中（新表 + CRUD） | 小（加一個 column） | 小（加幾個 column） |
| 圖片數量彈性 | 無上限 | 無上限 | 固定上限 |
| 排序/主圖標記 | 容易（sort_order, is_primary） | 困難 | 困難 |
| 查詢效能 | JOIN 但走 index 很快 | JSON 解析有開銷 | 最快（直接欄位） |
| 資料庫正規化 | 符合（一對多正規關聯） | 不符合（陣列塞一個欄位） | 不符合（重複結構） |
| 跟現有系統一致性 | **跟 event_image 一致** | — | — |
| 未來擴展性 | 高 | 中 | 低 |
| MySQL JSON 索引支援 | 好（直接加 index） | 差（需 generated column） | 好（直接加 index） |

### 建議

**做法 1（一對多關聯 + 多態圖片表）**最適合我們，理由：
...
6. 搭配[議題三](#議題三通用化-event_image-表--多態圖片表)的多態圖片表，用單一 `image` 表統一管理所有實體的圖片

做法 2（JSON）只適合簡單展示、不需要排序和主圖標記的場景，改動最小但擴展性差。做法 3（多個固定欄位）不建議，太不彈性。
...
CREATE TABLE image (
    ...
    -- 不加 CHECK 約束（見下方說明）
    -- entity_type 合法值由 service 層驗證，不在 DB 層限制
);
```
...
> image_type 的值可以跨 entity_type 共用（如 `banner`、`gallery`），也可以有特定的值（如 `logo` 只有 company 用）。image_type 不加 CHECK 約束，保持彈性。

### 為什麼不加 CHECK 約束？

多態圖片表的 `entity_type` **不用** DB 層 CHECK 約束，改由 service 層驗證。理由：

1. **每次新增 entity_type 就要 ALTER TABLE** — MySQL/MariaDB 修改 CHECK 約束會鎖表，造成停機風險
2. **主流 ORM 都不產生 CHECK** — Rails ActiveRecord、Go GORM、Laravel Eloquent 的多態關聯都不加 CHECK，靠 application 層驗證
3. **SQL 本質上無法完整表達多態約束** — entity_type + entity_id 的外鍵本來就無法用 FK 約束，CHECK 也只是部分保護
4. **業界共識是 application 層驗證** — 不是妥協，而是多態模式的正確做法

**參考文獻**：
- [Rails Guides — Polymorphic Associations](https://guides.rubyonrails.org/association_basics.html): Rails 不對 polymorphic type 加任何 DB 約束
- [GORM — Polymorphism](https://gorm.io/docs/polymorphism.html): Go GORM 不產生 CHECK constraint
- [DoltHub — Choosing a Database Schema for Polymorphic Data](https://www.dolthub.com/blog/2024-06-25-polymorphic-associations/): CHECK 約束「only grow in complexity」且「error message isn't very helpful」
- [GitLab Docs — Polymorphic Associations](https://docs.gitlab.com/development/database/polymorphic_associations/): GitLab 不建議用 CHECK，type column 靠程式碼驗證
- [strong_migrations (GitHub)](https://github.com/ankane/strong_migrations): 修改 CHECK 約束在 MySQL 是 unsafe migration，會阻塞寫入
- [TypeDB Blog — Inheritance and Polymorphism in SQL](https://typedb.com/blog/inheritance-and-polymorphism-where-the-cracks-in-sql-begin-to-show): 「All three design patterns have resulted in business logic being moved from the database to the application」
