...
**壞處：**
- ~~`entity_id` 沒辦法加外鍵約束（指向不同表）~~ → 見[建議](#建議)：log 表本來就是紀錄性質，原始資料被刪了 log 保留也合理，外鍵約束不必要
- ~~純紀錄型的資料會有一堆空的 status/reviewer 欄位~~ → 見[議題二](#議題二純紀錄的-null-欄位有差嗎)：MySQL InnoDB 的 NULL 每欄位只佔 1 bit，100 萬筆 < 1 MB，不是問題
- 單表資料量大，要注意 index → 見[效能分析](#效能分析合併-vs-拆分在大資料量下的表現)：100 萬筆以內毫秒級，千萬筆以下不是瓶頸
