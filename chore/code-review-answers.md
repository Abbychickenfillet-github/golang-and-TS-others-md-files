---
title: "code-review-answers"
---

# 程式碼審查問題回答

## 1. 目前有設定預設的規則嗎？如果有的話在哪？為什麼用"預設規則"這個字眼？

**回答**：
- ❌ **沒有預設的規則**。我錯誤地假設 `company_id IS NULL` 是"預設規則"，但實際上：
  - `company_id` 可以是 `NULL`（不綁定特定公司）
  - `company_id` 也可以綁定特定公司（如 GC 或 electricity_company）
  - 沒有設計說明 `company_id IS NULL` 是"預設規則"
- **修復**：移除"預設規則"的概念，直接查找活動的有效規則（`status='active'` 且 `expired_at IS NULL`）

## 2. status='active' 和 deleted_at 是一樣的用途，檢查哪個用的比較多，刪除哪一個？

**回答**：
- **檢查結果**：
  - `status` 在程式碼中大量使用（639行）
  - `deleted_at` 在程式碼中大量使用（169行）
