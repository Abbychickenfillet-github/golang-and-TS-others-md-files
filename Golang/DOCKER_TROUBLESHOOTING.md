...
● 為什麼你不用管內部連接埠 8080
  ...
  1. 你要改 Go 程式碼 - 修改監聽連接埠（99% 不需要）
  2. 你要改 Dockerfile - 修改 EXPOSE 設定（99% 不需要）
  3. 連接埠衝突 - 容器內有多個服務用同一連接埠（很少見）

  類比

  就像寄信：
  - 外部連接埠 8003 = 你家地址（你告訴別人的）
  - 內部連接埠 8080 = 你家信箱在幾樓（郵差自己知道，你不用管）
