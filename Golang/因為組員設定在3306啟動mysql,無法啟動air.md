---
title: "因為組員設定在3306啟動mysql,無法啟動air"
---
...
執行 `air` 時出現錯誤，因為 MySQL 連接埠 3306 被佔用。
...
2. 這啟動了一個 MySQL 容器，佔用了 **3306 連接埠**
3. 之後你執行 `air`
4. Go 程式嘗試連接資料庫時失敗（連接埠衝突或配置不符）
...
修改 `docker-compose.dev.yml` 的連接埠（避免與其他服務衝突）：
