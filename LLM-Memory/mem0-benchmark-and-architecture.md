...
記憶不是隻抽不丟。Mem0 在每次新事實要寫進去前，會對比**既有記憶**做四種判斷：
...
有趣的是——**user prompt 和 agent prompt 看到的記憶 ID 列表，也是各自一套 `[0, 1, 2...]` 整數映射**，互不幹擾。LLM 每次只處理一種角色的事實，用最簡單的整數索引決策，系統再把整數對回真實 UUID。**「簡化 LLM 職責 + 簡化 LLM 看到的符號」** 是 Mem0 一以貫之的設計哲學。
